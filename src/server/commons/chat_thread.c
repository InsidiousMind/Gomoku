  ...
  if (rv == -1) {
    ...
    for (i = 0; i < conn_head->size; i++) {
      ...
      if (ufds[i].revents & POLLIN) { //data ready to be recved on this socket
        char peek[2];
        ssize_t read_count = recv(ufds[i].fd, &peek, sizeof(char) * 2,
        ...
        if (peek[0] == '\v') {
          ...
          if (errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            memmove(buf, buf + 2, strlen(
                buf)); //get rid of first two ('\v' and len(c_upid) chars)
            memcpy(&c_upid, &buf, (size_t) len_of_upid); //gets the UPID
            memmove(buf, buf + len_of_upid,
                    strlen(buf)); //gets rid of upid in the string we want
        ...
        //end of MSG_PEEK if
          ...
          POLLOUT) {  //this socket is ready to send data
